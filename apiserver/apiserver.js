...
app.post('/api/org/login', async function (req, res) {
    try {
        const ccpPath = path.resolve(__dirname, '..', '..', 'test-network', 'organizations', 'peerOrganizations', 'org1.example.com', 'connection-org1.json');
        const ccp = JSON.parse(fs.readFileSync(ccpPath, 'utf8'));
        ...
        const result = await contract.evaluateTransaction('IsValidOrg', req.body.username, req.body.password);
        ...
        if (result.toString() == "true") {
            ...
            res.redirect('/api/allprofiles')
        } else {
            res.render('org/login', { message: "Invalid user" })
        }
...
app.post('/api/v2/org/login', async function (req, res) {
    try {
        const ccpPath = path.resolve(__dirname, '..', '..', 'test-network', 'organizations', 'peerOrganizations', 'org1.example.com', 'connection-org1.json');
        const ccp = JSON.parse(fs.readFileSync(ccpPath, 'utf8'));
        // Create a new file system based wallet for managing identities.
...
app.post('/api/org/register', async function (req, res) {
    try {
        const ccpPath = path.resolve(__dirname, '..', '..', 'test-network', 'organizations', 'peerOrganizations', 'org1.example.com', 'connection-org1.json');
        const ccp = JSON.parse(fs.readFileSync(ccpPath, 'utf8'));
        // Create a new file system based wallet for managing identities.
        ...
        await contract.submitTransaction('updateOrgLogin', req.body.username, req.body.password);
        console.log('[Register] Transaction has been submitted');

        res.redirect('/api/org/login')
...
app.post('/api/org/profile/update', async function (req, res) {
    try {
        ...
        res.redirect('/api/org/profile/' + req.body.userid)
...
app.get('/api/org/profile/:car_index', async function (req, res) {
    try {
        const ccpPath = path.resolve(__dirname, '..', '..', 'test-network', 'organizations', 'peerOrganizations', 'org1.example.com', 'connection-org1.json');
        const ccp = JSON.parse(fs.readFileSync(ccpPath, 'utf8'));
        // Create a new file system based wallet for managing identities.
        ...
        // console.log(JSON.parse(result).data)
        console.log({ profile: JSON.parse(JSON.parse(result).data) })
        res.render("profile-org", { profile: JSON.parse(JSON.parse(result).data) });
    } catch (error) {
        console.error(`Failed to evaluate transaction: ${error}`);
        res.status(500).json({ error: error });
        process.exit(1);
